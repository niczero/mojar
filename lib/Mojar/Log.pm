...
sub format {
  ...
  return encode 'UTF-8', sprintf '%s[%s] %s',
      strftime($self->pattern, localtime), $level, join "\n", @lines, '';
}
...
ability to set defaults at 'use' time (via a helper) and a more ISO-ish
timestamp with support for customising that pattern.
...
parameters via an implicit helper.  In a standalone script this could be done
as:
...
Employing use-time parameters entails the implicit creation of a method (helper)
and a hash entry.  The helper works by assigning a Log object to the caller's
'log' hash key, so only works with objects implemented as hashrefs.  In other
words, only use the convenience of the helper if that's what you were going to
do anyway.  [Using the package name to hold a hashref is considered hacky and
makes debugging a little trickier; creating a log object per caller object is
inefficient in a lot of cases.  If either of those trouble you, please re-read
the intro to this section then return to the SYNOPSIS.]
